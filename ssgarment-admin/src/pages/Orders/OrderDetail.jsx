...
export default function OrderDetail() {
  ...
  const isCOD = (o) => o?.payment_mode === 'COD';
  const getPaymentBadge = (o) =>
    isCOD(o)
      ? 'bg-amber-50 text-amber-700 border-amber-200'
      : 'bg-emerald-50 text-emerald-700 border-emerald-200';
  const getPaymentLabel = (o) => (isCOD(o) ? 'Cash on Delivery' : 'Paid Online');
  const getCodBalance = (o) => Math.max(0, (o?.total_price || 0) - COD_ADVANCE);
  ...
  return (
    <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              {/* 🔹 Payment mode chip */}
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getPaymentBadge(order)}`}
              >
                {isCOD(order) ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-5 sm:p-6">
          <div className="flex items-center gap-2 mb-4">
            <div className={`w-6 h-6 rounded-md flex items-center justify-center ${isCOD(order) ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
              {isCOD(order) ? (
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
            ...
            <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wider">Payment</h2>
            <span className={`ml-auto inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getPaymentBadge(order)}`}>
              {getPaymentLabel(order)}
            </span>
          </div>
          ...
          {isCOD(order) ? (
            <div className="space-y-2.5">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-slate-800">₹{COD_ADVANCE}</span>
              </div>
              <div className="flex items-center justify-between text-sm bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5">
                <span className="text-amber-700 font-bold">Due on delivery</span>
                <span className="font-bold text-amber-700">₹{getCodBalance(order)}</span>
              </div>
              <p className="text-xs text-slate-400 pt-1">
                Collect ₹{getCodBalance(order)} in cash/UPI when handing over the order.
              </p>
            </div>
          ) : (
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              <span className="font-medium">Full amount paid via Razorpay</span>
            </div>
          )}
        </div>
      </div>
